  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">Phân tích tài chính 6 tháng gần nhất</p>
      </div>

      {/* KPIs */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white rounded-2xl border border-slate-200 p-5">
          <div className="text-sm text-slate-500 mb-1">Tổng doanh thu</div>
          <div className="text-2xl font-bold text-slate-900">{(totalRevenue / 1000000).toFixed(0)}M₫</div>
          <div className="text-xs text-green-600 font-semibold mt-1">+12.5% so với kỳ trước</div>
        </div>
        <div className="bg-white rounded-2xl border border-slate-200 p-5">
          <div className="text-sm text-slate-500 mb-1">Lợi nhuận gộp</div>
          <div className="text-2xl font-bold text-slate-900">{(totalProfit / 1000000).toFixed(0)}M₫</div>
          <div className="text-xs text-green-600 font-semibold mt-1">+9.8% so với kỳ trước</div>
        </div>
        <div className="bg-white rounded-2xl border border-slate-200 p-5">
          <div className="text-sm text-slate-500 mb-1">Biên lợi nhuận</div>
          <div className="text-2xl font-bold text-slate-900">{margin}%</div>
          <div className="text-xs text-slate-400 mt-1">Trung bình 6 tháng</div>
        </div>
      </div>

      {/* Area chart */}
      <div className="bg-white rounded-2xl border border-slate-200 p-6">
        <h2 className="text-base font-bold text-slate-900 mb-6">Doanh thu & Lợi nhuận</h2>
        <ResponsiveContainer width="100%" height={280}>
          <AreaChart data={monthlyData}>
            ...
            <Tooltip
              ...
              formatter={(value: number, name: string) => [`${(value / 1000000).toFixed(1)}M₫`, name === "revenue" ? "Doanh thu" : "Lợi nhuận"]}
            />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-2xl border border-slate-200 p-6">
          <h2 className="text-base font-bold text-slate-900 mb-6">Doanh thu theo danh mục</h2>
          <div className="flex items-center gap-6">
            <ResponsiveContainer width="50%" height={200}>
              <PieChart>
                ...
                <Tooltip formatter={(v: number) => [`${v}%`, "Tỷ trọng"]} contentStyle={{ borderRadius: "12px", fontSize: "12px" }} />
              </PieChart>
            </ResponsiveContainer>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6">
          <h2 className="text-base font-bold text-slate-900 mb-6">Top thương hiệu</h2>
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={topBrands} layout="vertical">
              <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" horizontal={false} />
              <XAxis type="number" tick={{ fontSize: 11, fill: "#64748b" }} tickFormatter={(v) => `${v / 1000000}M`} />
              <YAxis type="category" dataKey="brand" tick={{ fontSize: 12, fill: "#64748b" }} width={55} />
              <Tooltip
                contentStyle={{ backgroundColor: "#fff", border: "1px solid #e2e8f0", borderRadius: "12px", fontSize: "12px" }}

...
// ── Node content — memo'd so it only re-renders when its own props change ─────
...
const NodeContent = memo(function NodeContent({
  node, depth, expanded, isDragging, isDropTarget,
  handleRef, toggling,
  ...
  return (
    <div className={`rounded-xl transition-colors duration-100 ${isDropTarget ? 'bg-blue-50 ring-2 ring-blue-300' : ''} ${isDragging ? 'opacity-30' : ''}`}>
      <div
...
// ── Sortable wrapper — thin, only holds useSortable state ─────────────────────

function TreeNodeRow({ node, index, depth, expanded, toggling, onToggleExpand, onEdit, onDelete, onToggleActive }: {
  node: TreeNode; index: number; depth: number; expanded: boolean; toggling: boolean;
  onToggleExpand: () => void; onEdit: () => void; onDelete: () => void; onToggleActive: () => void;
}) {
...
export const CategoryTree = memo(function CategoryTree({
  nodes, depth, expandedIds,
  onToggleExpand, onEdit, onDelete, onToggleActive, togglingId,
}: CategoryTreeProps) {
  return (
    <>

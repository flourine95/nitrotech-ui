      ...
      <aside className="fixed top-0 right-0 z-50 flex h-full w-full max-w-md flex-col bg-card shadow-2xl" role="dialog" aria-modal="true">
        <div className="flex items-center justify-between border-b border-border px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-semibold text-foreground">{isEdit ? 'Sửa danh mục' : 'Thêm danh mục'}</h2>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-1 flex-col overflow-y-auto" noValidate>
          <div className="flex-1 space-y-5 px-6 py-5">
            ...
            <div>
              <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-muted-foreground">Danh mục cha</label>
              <select value={parentId ?? ''} onChange={(e) => setValue('parentId', e.target.value ? Number(e.target.value) : null)}
                className="w-full cursor-pointer rounded-xl border border-border bg-muted/50 px-3.5 py-2.5 text-sm outline-none transition-colors focus:border-ring focus:bg-background focus:ring-2 focus:ring-ring/20">
              ...
              {selectedParent && (
                <p className="mt-1.5 flex items-center gap-1 text-xs text-muted-foreground/70">
                  ...
                  Nằm trong: <span className="font-medium text-muted-foreground">{selectedParent.name}</span>
                </p>
              )}
            ...
            <div className="flex items-center justify-between rounded-xl border border-border bg-muted/50 px-4 py-3">
              <div>
                ...
                <p className="text-xs text-muted-foreground/70">Khách hàng có thể thấy danh mục này</p>
              </div>
          ...
          <div className="border-t border-border px-6 py-4">
            <div className="flex gap-3">
              <button type="button" onClick={onClose} className="flex-1 cursor-pointer rounded-xl border border-border py-2.5 text-sm font-semibold text-foreground/80 transition-colors hover:bg-muted/50">Hủy</button>

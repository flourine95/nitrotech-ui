  ...
  return (
    ...
    >
      {uploading ? (
            ...
            {progress.map(p => (
              <div key={p.name} className="flex items-center gap-2">
                ...
                <span className="w-3 shrink-0">
                  {p.done && (
                    <svg viewBox="0 0 24 24" className="h-3 w-3 text-green-500" fill="none" stroke="currentColor" strokeWidth="3">
        ...
        <>
          <div className={`flex h-12 w-12 items-center justify-center rounded-2xl transition-colors ${dragging ? "bg-indigo-100" : "border border-slate-200 bg-white shadow-sm"}`}>
          ...
          <div className="text-center">
            ...
            <p className="mt-0.5 text-xs text-slate-400">
              PNG, JPG, WebP, GIF · Tối đa 10MB · Folder: <span className="font-medium text-slate-600">{folder}</span>
            </p>
          </div>
        </>
      )}

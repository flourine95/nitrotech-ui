  ...
  return (
    <aside className="flex w-72 shrink-0 flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        <div>
          <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-400">Thông tin</p>
          <div className="space-y-2">
            {META_ROWS(asset).map(({ label, value }) => (
              <div key={label} className="flex items-start justify-between gap-2">
                <span className="shrink-0 text-xs text-slate-400">{label}</span>
                <span className="max-w-[60%] truncate text-right text-xs font-medium text-slate-700">{value}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          {COPY_FIELDS(asset).map(({ label, value, key }) => (
            <div key={key} className="flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2">

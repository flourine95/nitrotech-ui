...
function SortHeader({ label, field, sortBy, sortDir, onSort }: {
  ...
  sortDir: 'asc' | 'desc'; onSort: (f: string) => void;
}) {
...
export default function DashboardOrdersPage() {
  ...
  const [sortDir,     setSortDir]     = useQueryState('dir',  parseAsStringEnum<'asc'|'desc'>(['asc','desc']).withDefault('desc'));
  ...
  function openSheet() {
    ...
    setDraftRange(
      dateFrom ? { from: parseLocalDate(dateFrom), to: dateTo ? parseLocalDate(dateTo) : undefined } : undefined,
    );
    setSheetOpen(true);
  }
  ...
  function clearAllFilters() {
    startTransition(() => {
      void setSearch(''); void setStatuses(''); void setDateFrom(''); void setDateTo('');
      void setSortBy('createdAt'); void setSortDir('desc'); void setCurrentPage(0);
    });
  }

  // ── Queries ──
  const ordersQuery = useQuery({
    queryKey: ['orders', deferredSearch, statuses, dateFrom, dateTo, sortBy, sortDir, currentPage, pageSize],
    queryFn: () => Promise.resolve(mockOrdersPage({
  ...
  const summaryQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      return Object.fromEntries(ALL_STATUSES.map((s, i) => [s.value, results[i].totalElements])) as Record<OrderStatus, number>;
    },
    staleTime: 60_000,
  });
  ...
  const columns: ColumnDef<Order>[] = [
    {
      accessorKey: 'code',
      header: () => <SortHeader label="Mã đơn" field="code" sortBy={sortBy} sortDir={sortDir} onSort={handleSort} />,
      cell: ({ row }) => <span className="font-mono font-semibold">{row.original.code}</span>,
    },
    {
      accessorKey: 'customerName',
      header: () => <SortHeader label="Khách hàng" field="customerName" sortBy={sortBy} sortDir={sortDir} onSort={handleSort} />,
      cell: ({ row }) => (
    ...
    },
    {
      ...
      header: () => (
        <div className="flex justify-end">
          <SortHeader label="Tổng tiền" field="totalAmount" sortBy={sortBy} sortDir={sortDir} onSort={handleSort} />
        </div>
      ),
      meta: { className: 'text-right' },
      cell: ({ row }) => <div className="text-right font-semibold">{vnd.format(row.original.totalAmount)}</div>,
    },
    {
      accessorKey: 'createdAt',
      header: () => <SortHeader label="Ngày đặt" field="createdAt" sortBy={sortBy} sortDir={sortDir} onSort={handleSort} />,
      cell: ({ row }) => (
  ...
  const table = useReactTable({ data: orders, columns, getCoreRowModel: getCoreRowModel(), manualPagination: true, pageCount: totalPages });

  const { pages, showLeftEllipsis, showRightEllipsis } = usePagination({
    currentPage: currentPage + 1, totalPages, paginationItemsToDisplay: 5,
  });
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground text-sm">{totalElements} đơn hàng</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
        ...
        >
          <Badge variant="secondary" className="size-9 justify-center text-base font-bold tabular-nums">
            {summaryQuery.data ? Object.values(summaryQuery.data).reduce((a, b) => a + b, 0) : 0}
          </Badge>
          <span className="text-sm">Tất cả</span>
        </button>
        {ALL_STATUSES.map((s) => {
          const isActive = appliedStatuses.length === 1 && appliedStatuses[0] === s.value && !dateFrom;
          return (
            <button
              key={s.value}
              onClick={() => { setStatuses(s.value); setDateFrom(''); setDateTo(''); setCurrentPage(0); }}
              className={cn(
                'flex items-center gap-3 rounded-lg border bg-card p-4 text-left transition-colors hover:bg-accent',
                isActive && 'ring-2 ring-ring',
              )}
            >
              <Badge variant={statusConfig[s.value].variant} className="size-9 justify-center text-base font-bold tabular-nums">
          ...
          <div className="flex flex-wrap gap-1.5">
            {appliedStatuses.map((s) => (
              <Badge key={s} variant="secondary" className="gap-1 pr-1">
                ...
                <button
                  onClick={() => { setStatuses(appliedStatuses.filter((x) => x !== s).join(',')); setCurrentPage(0); }}
      ...
      {/* Table */}
      <div className={cn('rounded-lg border bg-card transition-opacity duration-150', (ordersQuery.isFetching && !ordersQuery.isLoading) || isPending ? 'opacity-60' : 'opacity-100')}>
        <Table>
          <TableHeader>
          ...
          <TableBody>
            {ordersQuery.isLoading ? (
              <TableRow>
                <TableCell colSpan={columns.length} className="h-32 text-center text-muted-foreground">Đang tải...</TableCell>
              </TableRow>
        ...
        <div className="flex flex-col gap-3 border-t px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          {totalPages > 1 && (
            <Pagination>
              <PaginationContent>
                <PaginationItem>
                  <Button variant="ghost" size="sm" onClick={() => startTransition(() => { void setCurrentPage(Math.max(0, currentPage - 1)); })} disabled={currentPage === 0}>
                ...
                {pages.map((page) => (
                  <PaginationItem key={page}>
                    <Button size="icon" variant={page === currentPage + 1 ? 'default' : 'outline'} onClick={() => startTransition(() => { void setCurrentPage(page - 1); })}>
                      {page}
                    </Button>
                  </PaginationItem>
                ))}
                ...
                <PaginationItem>
                  <Button variant="ghost" size="sm" onClick={() => startTransition(() => { void setCurrentPage(Math.min(totalPages - 1, currentPage + 1)); })} disabled={currentPage >= totalPages - 1}>
